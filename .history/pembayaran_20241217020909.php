...
if (isset($_GET['keranjang']) && $_GET['keranjang'] === session_id()) {
    ...
    $user_id = $_SESSION['user_id']; // Misalnya, user_id sudah disimpan di session saat login
    $metode_pembayaran = 'Transfer'; // Anda bisa menambahkan pilihan metode pembayaran sesuai kebutuhan
    $tanggal_pembayaran = date('Y-m-d H:i:s');

    $insertPembayaran = "INSERT INTO pembayaran (user_id, total_harga, metode_pembayaran, status_pembayaran, tanggal_pembayaran)
                         VALUES ($user_id, $totalHarga, '$metode_pembayaran', 'Belum Lunas', '$tanggal_pembayaran')";
    if ($koneksi->query($insertPembayaran)) {
        $pembayaran_id = $koneksi->insert_id; // ID pembayaran yang baru saja dimasukkan

        // Update keranjang menjadi lunas setelah pembayaran
        // Anda bisa menambahkan logika lain sesuai dengan kondisi yang Anda inginkan
    } else {
