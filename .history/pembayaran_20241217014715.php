<?php
// Koneksi database
require "koneksi.php";  // Ensure this line is included
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if (!in_array($status_pembayaran, ['pending', 'selesai', 'gagal'])) {
        echo "<script>alert('Status pembayaran tidak valid.');</script>";
        exit();
    }

    // Gunakan prepared statement untuk update status pembayaran
    $query = "UPDATE pembayaran SET status_pembayaran = ? WHERE id = ?";
    $stmt = $koneksi->prepare($query);
