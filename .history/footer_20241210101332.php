  <!--sosial start-->
    < class="container-fluid py-5 subscribe-content text-light">
      <div class="container">
        <h5 class="text-center mb-3">Temui Kami</h5>
        <div class="row justify-content-center">
          <di class="col-sm-1  d-flex justify-content-center mb-2 bounce-icon">
           <a href="" <i href="" class="fa-brands fa-instagram"></i>
          </div>
          <div class="col-sm-1  d-flex justify-content-center mb-2 bounce-icon">
            <i class="fa-brands fa-facebook"></i>
          </div>
          <div class="col-sm-1  d-flex justify-content-center mb-2 bounce-icon" >
            <i class="fa-brands fa-whatsapp"></i>
          </div>
          <div class="col-sm-1  d-flex justify-content-center mb-2 bounce-icon">
           <i class="fa-brands fa-youtube"></i>
          </div>
        </div>
    </div>
    <!--sosial end-->
    <!-- footer -->
     <div class="container-fluid py-3 bg-dark text-light">
        <div class="container d-flex justify-content-between">
            <label>&copy;2024 Kafe Saya</label>
            <label>Created By Herman</label>
        </div>
    </div>