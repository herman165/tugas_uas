...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_payment'])) {
    ...
    // Validasi user_id
    $userCheck = $koneksi->prepare("SELECT id FROM users WHERE id = ?");
    ...
    if ($userCheck->num_rows > 0) {
        ...
        $query = $koneksi->prepare("INSERT INTO pembayaran (user_id, total_harga, metode_pembayaran) VALUES (?, ?, ?)");
        $query->bind_param("ids", $user_id, $total_harga, $metode_pembayaran);
        ...
        header("Location: pembayaran.php?status=user_not_found");
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    $updateQuery = $koneksi->prepare("UPDATE pembayaran SET status_pembayaran = ? WHERE id = ?");
    $updateQuery->bind_param("si", $status_pembayaran, $id);

    if ($updateQuery->execute()) {
        header("Location: pembayaran.php?status=updated");
    } else {
        header("Location: pembayaran.php?status=update_error");
    }
}
?>
    ...
    <div class="container mt-4">
        ...
        <?php if (isset($_GET['status'])): ?>
            <div class="alert 
                ...
                <?php
                switch ($_GET['status']) {
                    case 'success': echo "Pembayaran berhasil ditambahkan!"; break;
                    case 'updated': echo "Status pembayaran berhasil diperbarui!"; break;
                    case 'error': echo "Terjadi kesalahan saat menambahkan pembayaran."; break;
                    case 'user_not_found': echo "User ID tidak ditemukan."; break;
                    case 'update_error': echo "Terjadi kesalahan saat memperbarui status."; break;
                }
                ?>
            </div>
        <?php endif; ?>
